...
#define LOG(severity) cnlog::CLOG(CNNL, severity)
...
#define LOG_FIRST_N(severity, n)                                           \
  ...
  if (CNNL_PREDICT_FALSE(TOKENPASTE2(LOG_, __LINE__, _OCCURRENCES)++ < n)) \
  cnlog::CLOG(CNNL, severity)
...
// queue, errors occur earlly will be caught at cnnl function call. It's not a good
...
class LogMessageFatal : public LogMessage {
 public:
  LogMessageFatal(const char *file, int line) CNNL_ATTRIBUTE_COLD;
  CNNL_ATTRIBUTE_NORETURN ~LogMessageFatal();
};

// Otherwise, set CNNL_MIN_VLOG_LEVEL environment to update minimum log level
// of VLOG, or CNNL_CPP_VMODULE to set the minimum log level for individual
// translation units.
#define VLOG_IS_ON(lvl)                                               \
  (([](int level, const char *fname) {                                \
    static const bool vmodule_activated =                             \
        ::cnnl::internal::LogMessage::VmoduleActivated(fname, level); \
    return vmodule_activated;                                         \
  })(lvl, __FILE__))

#define VLOG(level)                     \
  CNNL_PREDICT_TRUE(!VLOG_IS_ON(level)) \
  ? (void)0 : ::cnnl::internal::Voidifier() & LOG(VLOG)
...
struct CheckOpString {
  ...
  operator bool() const { return CNNL_PREDICT_FALSE(str_ != NULL); }
  std::string *str_;
};
...
std::string *MakeCheckOpString(const T1 &v1,
                               const T2 &v2,
                               const char *exprtext) CNNL_ATTRIBUTE_NOINLINE;
...
#define CNNL_DEFINE_CHECK_OP_IMPL(name, op)                                             \
  ...
  inline std::string *name##Impl(const int v1, const size_t v2, const char *exprtext) { \
    if (CNNL_PREDICT_FALSE(v2 >= std::numeric_limits<int>::max())) {                    \
      return ::cnnl::internal::MakeCheckOpString(v1, v2, exprtext);                     \
    }                                                                                   \
...
// yacc grammar.
CNNL_DEFINE_CHECK_OP_IMPL(Check_EQ, ==)
CNNL_DEFINE_CHECK_OP_IMPL(Check_NE, !=)
CNNL_DEFINE_CHECK_OP_IMPL(Check_LE, <=)
CNNL_DEFINE_CHECK_OP_IMPL(Check_LT, <)
CNNL_DEFINE_CHECK_OP_IMPL(Check_GE, >=)
CNNL_DEFINE_CHECK_OP_IMPL(Check_GT, >)
#undef CNNL_DEFINE_CHECK_OP_IMPL

// In optimized mode, use CheckOpString to hint to compiler that
// the while condition is unlikely.
#define CHECK_OP_LOG(name, op, val1, val2)                                            \
  while (::cnnl::internal::CheckOpString _result = ::cnnl::internal::name##Impl(      \
             ::cnnl::internal::GetReferenceableValue(val1),                           \
             ::cnnl::internal::GetReferenceableValue(val2), #val1 " " #op " " #val2)) \
